...
def main(json_path='./options/train.json'):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            train_set = SemiSupDataset(dataset_opt)
            
            domain_frequency = dataset_opt['domain_frequency']
            ...
            for class_id, count in zip(unique, counts):
                weights[labels == class_id] = len(weights) * domain_frequency[str(class_id)]/count
                class_info = 'class: {}, count: {}, total: {}, weight: {}'.format(class_id, count, len(weights), len(weights) * domain_frequency[str(class_id)]/count)
            ...
            test_set = SemiSupDataset(dataset_opt)
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                for test_data in test_loader:
                    idx += 1
                    image_name_ext = os.path.basename(test_data['path_H'][0])
                    ...
                    img_G = util.tensor2uint(visuals['img_G'])
                    ...
                    save_img_E_path = os.path.join(img_dir, '{:s}_{:d}_E.png'.format(img_name, current_step))
                    
                    util.imsave(img_H, save_img_H_path)
                    util.imsave(img_L, save_img_L_path)
                    util.imsave(img_G, save_img_G_path)
                    util.imsave(img_E, save_img_E_path)
