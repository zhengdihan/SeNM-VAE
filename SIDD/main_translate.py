...
def main():
    ...
    parser.add_argument('--nls', nargs='+', type=int, default=[2, 4])
    parser.add_argument('--img_size', type=int, default=256, help='Image size')
    parser.add_argument('--num_down', type=int, default=0, help='downsample number')
    parser.add_argument('--zn_dim', type=int, default=16)
    ...
    for img in tqdm(H_paths):
        ...
        if (idx + 1) % batch_size == 0:            
            input_noise_levels = random_noise_levels_sidd(img_Hs.shape[0]).to(device) / scale
            img_Hs = img_Hs / scale

            img_Gs = model.translate(img_Hs, input_noise_levels, temperature=1.0)
            img_Gs = img_Gs * scale
            img_Hs = img_Hs * scale
            
            if normalize:
                img_Gs = img_Gs - torch.mean(img_Gs, dim=(2, 3), keepdim=True) + torch.mean(img_Hs, dim=(2, 3), keepdim=True)
