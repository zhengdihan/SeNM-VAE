...
clean_patchDir = os.path.join(tar, 'clean')
...
total_size = len(all_clean_files)

sub_clean_files = np.random.choice(all_clean_files, size=int(total_size // 2), replace=False, p=None)

sub_noisy_files = list(set(all_clean_files) - set(sub_clean_files))
sub_noisy_files = [sub_noisy_file.replace('clean', 'noisy') for sub_noisy_file in sub_noisy_files]

sub_clean_root = f'{tar}/sub_clean/'
sub_noisy_root = f'{tar}/sub_noisy/'

os.system('mkdir -p ' + sub_clean_root)
os.system('mkdir -p ' + sub_noisy_root)

for sub_clean_file in sub_clean_files:    
    shutil.copy(sub_clean_file, sub_clean_root)

for sub_noisy_file in sub_noisy_files:    
    shutil.copy(sub_noisy_file, sub_noisy_root)
